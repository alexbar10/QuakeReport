package com.alexbar10.quakereport.utils
...
class QueryUtils {
    companion object {
         ...
         */
        fun extractEarthquakes(): List<Earthquake> {

            // Create an empty ArrayList that we can start adding earthquakes to
            val earthquakes = ArrayList<Earthquake>()

            val root = JSONObject(SAMPLE_JSON_RESPONSE)
            val earthquakesJSON = root.optJSONArray("features")

            if (earthquakesJSON != null) {
                for (i in 0 until (earthquakesJSON.length()-1)) {
                    val earthquakeJson = earthquakesJSON.optJSONObject(i)
